...
async function apiSessionlessFetch(endpoint, options = {}, router){
    const response = await fetch(`${HOST}${endpoint}`, {
        ...options,
        headers: {
            'Content-Type': 'application/json',
...
export const Api = {
    async login(name, router) {
        return apiSessionlessFetch(`/login`, {
            method: 'POST',
    ...
    async authenticate(name, password, router) {
        return apiSessionlessFetch(`/authenticate`, {
            method: 'POST',
    ...
    async createPassword(name, password, router) {
        return apiSessionlessFetch(`/createPassword`, {
            method: 'POST',
            body: JSON.stringify({'username': name, 'password': password})
        }, router)
    },
